...
export async function GET(ctx: Context): Promise<Response> {
  ...
  const whereClause = eq(schemas.industryTrading.period, period)
  const dataPromise = Database.select()
    .from(schemas.industryTrading)
    .where(whereClause)
    .limit(limit)
    .offset(offset)
  const countPromise = includeTotal
    ? Helpers.getTotalCount(Database, schemas.industryTrading, whereClause)
    : undefined
