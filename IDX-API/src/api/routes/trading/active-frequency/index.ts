...
export async function GET(ctx: Context): Promise<Response> {
  ...
  const whereClause = eq(schemas.activeFrequency.period, period)
  const dataPromise = Database.select()
    .from(schemas.activeFrequency)
    .where(whereClause)
    .limit(limit)
    .offset(offset)
  const countPromise = includeTotal
    ? Helpers.getTotalCount(Database, schemas.activeFrequency, whereClause)
    : undefined
