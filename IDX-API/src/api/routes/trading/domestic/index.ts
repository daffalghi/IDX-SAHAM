...
export async function GET(ctx: Context): Promise<Response> {
  ...
  const whereClause = eq(schemas.domesticTrading.period, period)
  const dataPromise = Database.select()
    .from(schemas.domesticTrading)
    .where(whereClause)
    .limit(limit)
    .offset(offset)
  const countPromise = includeTotal
    ? Helpers.getTotalCount(Database, schemas.domesticTrading, whereClause)
    : undefined
