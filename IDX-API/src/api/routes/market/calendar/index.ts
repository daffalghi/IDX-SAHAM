...
export async function GET(ctx: Context): Promise<Response> {
  ...
  const whereClause = and(
    gte(schemas.marketCalendar.date, dayStartMs),
    lt(schemas.marketCalendar.date, dayEndMs)
  )
  const dataPromise = Database.select()
    .from(schemas.marketCalendar)
    .where(whereClause)
    .limit(limit)
    .offset(offset)
  const countPromise = includeTotal
    ? Helpers.getTotalCount(Database, schemas.marketCalendar, whereClause)
    : undefined
