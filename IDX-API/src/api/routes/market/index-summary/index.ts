...
export async function GET(ctx: Context): Promise<Response> {
  ...
  const whereClause = and(
    gte(schemas.indexSummary.date, dayStartMs),
    lt(schemas.indexSummary.date, dayEndMs)
  )
  const dataPromise = Database.select()
    .from(schemas.indexSummary)
    .where(whereClause)
    .limit(limit)
    .offset(offset)
  const countPromise = includeTotal
    ? Helpers.getTotalCount(Database, schemas.indexSummary, whereClause)
    : undefined
